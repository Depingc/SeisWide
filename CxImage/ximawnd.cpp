...
long CxImage::Draw2(HDC hdc, long x, long y, long cx, long cy)
{
	...
	if (!bTransparent){
		...
		SetStretchBltMode(hdc,COLORONCOLOR);	
#endif
		i = StretchDIBits(hdc, x, y, cx, cy, 
			0, 0, head.biWidth, head.biHeight,
			info.pImage,
			(BITMAPINFO*)pDib, 
			DIB_RGB_COLORS,
			SRCCOPY);
...
long CxImage::Stretch(HDC hdc, long xoffset, long yoffset, long xsize, long ysize, DWORD dwRop)
{
	...
	//ysize = !isFacingDown  ?  -abs(ysize)  :  abs(ysize);
	...
	// NOTE: biHeight negative => Facing Down
	//       biHeight positive => Facing up
	//head.biHeight = ((BITMAPINFO*)pDib)->bmiHeader.biHeight = 
	//	!isFacingDown  ?  -abs(head.biHeight)  :  abs(head.biHeight);
	...
	StretchDIBits(hdc, 
			...
			ysize, 
					...
					0, // in bitmap pixels, not related to any client coords
					0, 
					head.biWidth, 
					abs(head.biHeight),
	...
	BITMAPINFO * pBi = (BITMAPINFO*)pDib;
		...
		int x= ((BITMAPINFO*)pDib)->bmiHeader.biWidth;
		//int y= ((BITMAPINFO*)pDib)->bmiHeader.biHeight;
		//((BITMAPINFO*)pDib)->bmiHeader.biHeight = -y;
		//int res = StretchDIBits(hdc, 
		//	xoffset, yoffset, 90000, 90000, 
		//	0, // in bitmap pixels, not related to any client coords
		//	0, head.biWidth, head.biHeight,
		//	info.pImage,(BITMAPINFO*)pDib,DIB_RGB_COLORS,dwRop);
