...
typedef struct tagCxImageInfo {
	...
	float*	pImageFloatDeping;				///< THE IMAGE BITS

	long bitspersampleDeping;  // used for 32 bit tiff, by Deping
	tagCxImageInfo() {
		pImageFloatDeping = NULL;
		bitspersampleDeping = 8; // default by the original CxImage packaging.
	}
	~tagCxImageInfo() {
		if (pImageFloatDeping) {
			free(pImageFloatDeping);
			pImageFloatDeping = NULL;
		}
	}
...
//
//#define DEP_NFLTS 29999
//union T2_4 {
//	float data[DEP_NFLTS];
//	int  dataint[DEP_NFLTS];
//	short  datashort[DEP_NFLTS * 2];
//	byte databyte[DEP_NFLTS * 4];
//};
