		...
		image->cmpts_[1]->prec_ == image->cmpts_[2]->prec_ )
	{
		...
		for (cmptno=0; cmptno<=info.nFrame; cmptno++) {
			...
			if (nshift > 0) {
				if (this->pAlpha)
					free(pAlpha);
				this->pAlpha = (BYTE *)malloc(info.dwEffWidth * h);
			}
			// end Deping revision
			...
			for (y = 0; y<h; y++) {
				...
				for (x=0; x<w; x++){
					...
					//	int sdfsdf = 1;
					//}
					size_t ii = x + (h - 1 - y)*info.dwEffWidth;
					//if (ii > info.dwEffWidth * h - 1) {
					//	int itest = 1;
					//}

					int_fast32_t a = jas_matrix_getv(bufs[0], x);
					if (nshift > 0)
						*(pAlpha + ii) = (BYTE)a;

					SetPixelIndex(x, h - 1 - y, (BYTE)(a >> nshift));
				}
			}
		}
	}
