...
////////////////////////////////////////////////////////////////////////////////
bool CxImageTIF::DecodeExDeping32(TIFF* ptif)
{
	if (info.bitspersampleDeping <= 8 || info.bitspersampleDeping > 32) {
		return true;
	}
	if (!ptif) return false;

	// by now, the bitspersample is >8 bits, so we will use   *info.pImageFloatDeping
	uint32 height = 0;
	uint32 width = 0;
	uint16 bitspersample = 1;
	uint16 samplesperpixel = 1;

	TIFFGetField(ptif, TIFFTAG_BITSPERSAMPLE, &bitspersample);
	...
	if (info.pImageFloatDeping) free(info.pImageFloatDeping);
	info.pImageFloatDeping = (float *)malloc(width*height*sizeof(float));
	if (info.bitspersampleDeping == 16) {
		WORD* rowbuff = (WORD *)_TIFFmalloc(TIFFScanlineSize(ptif)); // this is imp !!
		for (register UINT row = 0; row <height; row++) {
			register int readlen = TIFFReadScanline(ptif, rowbuff, height - row, 0);
			// there is another function in libtiff which multiple lines at a time, try that as well
			for (register UINT col = 0; col < width; col++) {
				*info.pImageFloatDeping = (float)rowbuff[col];
				info.pImageFloatDeping++;
			}
		}
		free(rowbuff);
	}
	else if (info.bitspersampleDeping == 32) {
		UINT* rowbuff = (UINT *)_TIFFmalloc(TIFFScanlineSize(ptif)); // this is imp !!
		for (register UINT row = 0; row <height; row++) {
			register int readlen = TIFFReadScanline(ptif, rowbuff, height - row, 0);
			// there is another function in libtiff which multiple lines at a time, try that as well
			for (register UINT col = 0; col < width; col++) {
...
bool CxImageTIF::Decode(CxFile * hFile)
{
  ...
  cx_try
  {
	...
	info.bitspersampleDeping = bitspersample;  // save it; by Deping
	...
	if (info.bitspersampleDeping > 8) {
		this->DecodeExDeping32(m_tif);
	}
