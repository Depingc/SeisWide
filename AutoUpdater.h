...
#define LOCATION_UPDATE_FILE_CHECK _T("glupdate.txt")
...
public:
	...
	ErrorType CheckForUpdateOld(LPCTSTR UpdateServerURL);	
	ErrorType CheckForUpdateOld2(LPCTSTR UpdateServerURL);
	ErrorType CheckForUpdate(LPCTSTR UpdateServerURL);
	ErrorType JustUpdate(CString UpdateServerURL, CString);

	CString GetUrlContents(CString url);
	...
	const CString GetFileVersion(LPCTSTR file);
	const int		CompareVersions(CString ver1, CString ver2);
	const bool	IsDigits(CString text);
	const CString GetExecutable();
