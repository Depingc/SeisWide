...
{
	//const unsigned char* pc = (const unsigned char *) szInput.GetBuffer(0);
 // std::string encoded = base64_encode(reinterpret_cast<const unsigned char*>(szInput), s.length());
 ...
 //   ZeroMemory(pOutput, nLeng);
	...
	szOutput = CString( base64_encode((unsigned char const*)pInput, strlen(pInput)).c_str() );
    // szOutput = CA2T(pOutput);
    delete []pInput;
  //  delete []pOutput;
	return szOutput.GetLength();
}

int CStringDecodeBase64(const CString &szInput, CString &szOutput)
{
	std::string s = CT2A(szInput);
	szOutput = CString( base64_decode(s).c_str() );
	return szOutput.GetLength();
}
