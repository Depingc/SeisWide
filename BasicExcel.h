// THIS CLASS IS NOT USED IN SEISWIDE
...
protected:
	...
	public:
		...
		WORD uk5_;					// Unknown constant (revision?) (0x0018)
		WORD uk6_;					// Unknown constant	(version?) (0x001A)
		WORD uk7_;					// Unknown constant (0x001C)
		WORD log2BigBlockSize_;	// Log, base 2, of the big block size (0x001E)
	...
	public:
		...
		WORD nameSize_;				// Number of characters in the NAME field (0x40)
...
public:
	...
	struct Window1 : public Record
	{
		...
		WORD height_;
		WORD options_;
		WORD activeWorksheetIndex_;
		WORD firstVisibleTabIndex_;
		WORD selectedWorksheetNo_;
		WORD worksheetTabBarWidth_;
	};
	struct Backup;
	struct HideObj;
	struct DateMode;
	...
	struct XF : public Record
	{
		...
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);	
		WORD fontRecordIndex_;
		WORD formatRecordIndex_;
		WORD protectionType_;
		char alignment_;
		char rotation_;
...
public:
	...
	struct Dimensions : public Record
	{
		...
		WORD firstUsedColIndex_;
		WORD lastUsedColIndexPlusOne_;
		WORD unused_;		
	};
	struct CellTable
	{
		struct RowBlock
		{
			struct Row : public Record
			{
				...
				virtual size_t Write(char* data);	
				WORD rowIndex_;
				WORD firstCellColIndex_;
				WORD lastCellColIndexPlusOne_;
				WORD height_;
				WORD unused1_;
				WORD unused2_;
				int options_;
			...
			struct CellBlock
			{
				...
				struct Formula : public Record
				{
					...
					struct Table : public Record
					{
						...
						char lastColIndex_;
						WORD options_;
						WORD inputCellRowIndex_;
						WORD inputCellColIndex_;
						WORD inputCellColumnInputRowIndex_;
						WORD inputCellColumnInputColIndex_;
					};
	...
	struct Window2 : public Record
	{
		...
		virtual size_t Write(char* data);	
		WORD options_;
		WORD firstVisibleRowIndex_;
		WORD firstVisibleColIndex_;
		WORD gridLineColourIndex_;
		WORD unused1_;
		WORD magnificationFactorPageBreakPreview_;
		WORD magnificationFactorNormalView_;
		int unused2_;		
	};
	struct SCL;
	struct Pane;
...
public: // File functions.
	...
	bool SaveAs(CString filename);	///< Save current Excel workbook to a file.
