...
int CGZip::ReadString(unsigned char** depString)
{
	using namespace std;
	int read;
	...
	// allocating memory and writing buffer	
	*depString=new unsigned char[nBytes+1];
	size_t offset=0;
	while (!lBuffers.empty())
	{
		pBuffer=lBuffers.front().first;
		read=lBuffers.front().second;
		memcpy((*depString)+offset, pBuffer, read);
		offset+=read;
	...
	//CString t = CString(CA2T(depBuffer));
	//**ppString = CA2T(depBuffer);
	//delete[] depBuffer;
