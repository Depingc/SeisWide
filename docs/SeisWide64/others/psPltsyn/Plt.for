...
c....end plot bondaries...

        if(ibsmth.eq.2) then
c....plot smoothed boundaries.               
          call pcolor(mcol(4))
c
          do 600 i=1,npbnd 
...
c.....end plot model boundaries.

c...plot vel values. I don't need all t he following.
      if(ivel.ne.0) then
c
        call pcolor(mcol(5))
c
...
c
     ...
     +                     xbmin,xbmax,tbmin,tbmax,itxbox)
...
41                   continue
     ...
     +           and.itx.lt.3) call ssymbl(x(1),t(1),symht,1)
               end if
             else 
               ih=0
               if(itxout.eq.1.or.itxout.eq.2) then
...
c
      ...
      if(ip.eq.0) then
        ...
        do 10 i=1,nshot
           ...
           if(xdiff.lt..001.or.isep.lt.2) then
...
c...added by Deping: store ray path info for SeisWide plotting.
	write(88)npts
	write(88)(xr(i),i=1,npts)
	write(88)(zr(i),i=1,npts)
c...end of Deping's addition.
c
      do 10 i=1,npts
