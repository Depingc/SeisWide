...
c.... fix by Deping: to avoid program crash of out of array bounds      
      if (n1 .gt. pitan2) then
        f(1) = 0.
        f(2) = 0.
      else
      ...
      term2=x*(cg(layer,iblk,1)+cg(layer,iblk,2)*x)+
     +       term1*y(1)+cg(layer,iblk,5)
      vxv=(x*(cg(layer,iblk,8)+cg(layer,iblk,9)*x)+
...
c.... fix by Deping: to avoid program crash of out of array bounds      
      ...
      term2=y(1)*(cg(layer,iblk,1)+cg(layer,iblk,2)*y(1))+
     +       term1*x+cg(layer,iblk,5)
      vxv=(y(1)*(cg(layer,iblk,8)+cg(layer,iblk,9)*y(1))+
