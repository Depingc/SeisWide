...
c     
c     Automatic search mode routines for TRAMP
c                 
c     ----------------------------------------------------------------
c                 
...
c
      ...
      idray(2)=1  
c
      call trace(npt,ifam,ir,iturn,0,0,iflag2,0,idr)
c
...
c                 
     ...
     +           ia0,stol,irays,nskip,idot,irayps,xsmax,istep)
c                 
...
c     bottom of the layer directly beneath the shot point
c     in which rays are to be traced 
c 
      include 'tramp.par'
      include 'tramp.com'                
c                 
...
c note: since this routine only calculates min and max velocity in 
c a given layer,
c..... vti and vbi are the velocities for the top and bottom of layer i    
c......l   is the lowerest layer number for this ray
              if (is_anisotropy.gt.0 .and. anisotropy(i) .gt. 0.) then
                    vti = vti * (1. - anisotropy(i))
                    vbi = vbi * (1. - anisotropy(i))
              endif
...
20       continue 
        ...
        if (i.eq.l .and. 
     +      is_anisotropy.gt.0 .and. anisotropy(i) .gt. 0.) then
            vbotom = vbotom * (1. + anisotropy(i))
        endif
