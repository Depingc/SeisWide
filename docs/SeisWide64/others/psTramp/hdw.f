...
c
c....deping's fix: last two arguments not used in subroutine.                 
c      call ttime(isw,xshot,npt,nr,a1,ifam,itt,iszero,iamp,2,idump,
c     +           omega)
      call ttime(isw,xshot,npt,nr,a1,ifam,itt,iszero,iamp,2)
      qh=qa       
...
1000  if(ibsmth.eq.0) then
     ...
     +        /xsinc+cosmth(layer+1,npl)
      ...
      if(abs(abnd-alpha).le.angbnd) then
        ...
        ar(1,2)=fid*a-alpha
        npt=1     
        call trace(npt,ifam,nr,iturn,iamp,ihamp,iflag,idl,idr)
c                 
        qa=qh     
        call ttime(isw,xshot,npt,nr,a1,ifam,itt,iszero,iamp,iflag2)
...
c      
      ...
      ichbnd=iccbnd
c                 
      call ttime(isw,xshot,npt,nr,a1,ifam,itt,iszero,0,2)
      if(iray.ne.0) call pltray(npt,nskip,idot,irayps,0,0.)
c                 
      t1=time     
      l1=layer+1  
