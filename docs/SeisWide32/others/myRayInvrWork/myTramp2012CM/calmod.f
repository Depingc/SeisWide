...
c.... This routine is written by Deping Chian.... Apr, 2010
      function getVelAniRatio(ang)
            include 'tramp.par'
            include 'tramp.com' 
c...Out of rayinvr.par ,  This function uses only pi18
            angInDegree = abs(ang)*pi18
            if (angInDegree .gt. 90.0000) then
                angInDegree = 180. - angInDegree
             endif   
            if (angInDegree .lt. 0.0000) then
                angInDegree = -angInDegree
             endif   
            if (angInDegree .gt. 90.0000) then
                angInDegree = 180. - angInDegree
             endif   
...
c....for weak anisotropy media, we use a formula from:
c http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.64.908&rep=rep1&type=pdf
!              epsilon = 1. - anisotropy(i) * anisotropy(i)
!              angInPi = angInDegree / pi18
!              acos2 = cos(angInPi)**2
!              asin2 = sin(angInPi)**2
!              delta = 0.01
!c   abs(delta) <= 0.2 in its examples
!              bratio2 = delta * acos2*asin2 + epsilon*acos2*acos2
!              bratio = sqrt(1. - bratio2)
c          bratio = 1. - anisotropy(i) * (1.-angInDegree / 90.)
c          bratio = 1. - anisotropy(i) * angInDegree / 90.
ccccccccccc      angle=fid*(90.-ang)/pi18



            getVelAniRatio = 1.
c....  the final velocity shall be performed after calling this function
c           Vfinal = vel * (1 - anisotropy(i) * getVelAniRatio )


c....angInDegree=0   ->   getVelAniRatio = 100%
c....angInDegree=70  ->   getVelAniRatio = 90%
c....angInDegree=90  ->   getVelAniRatio = 0%
            if (angInDegree .lt. 70.) then
                getVelAniRatio = 1. - angInDegree / 70. * 0.1
            else
                getVelAniRatio = 0.9 - (angInDegree-70.) / 20. * 0.9
            endif
...
c....angInDegree=0   ->   getVelAniRatio = 100%
c....angInDegree=30  ->   getVelAniRatio = 90%
c....angInDegree=70  ->   getVelAniRatio = 80%
c....angInDegree=80  ->   getVelAniRatio = 10%
c....angInDegree=90  ->   getVelAniRatio = 0%
!            if (angInDegree .lt. 40.) then
!                getVelAniRatio = 1. - angInDegree / 40. * 0.1
!            else if (angInDegree .lt. 70.) then
!                getVelAniRatio = 0.9 - (angInDegree-40.) / 30. * 0.1
!            else if (angInDegree .lt. 80.) then
!                getVelAniRatio = 0.8 - (angInDegree-70.) * 0.1 * 0.7
!            else
!                getVelAniRatio = 0.1 - (angInDegree-80.) * 0.1 * 0.1
!            endif


!            getVelAniRatio = (cos(angInDegree/90. * pi)+1) *0.5



!            getVelAniRatio = getVelAniRatio * getVelAniRatio
          return      
      end         
c                 
...
c..... ang_this is the angle in radians to the vertical axis
c.....NOTE: 
c        when the angle is 90 degrees, it means the ray is traveling horizontally.
      if (is_anisotropy.gt.0 .and. anisotropy(i) .gt. 0. 
     +       .and. ang_this .ge. 0.) then

          bratio = 1.-anisotropy(i) * getVelAniRatio(ang_this)
ccccccccccc      angle=fid*(90.-ang)/pi18
      endif
      ...
      if (k .eq. 1) then
            cg=s2*(xb2*v1-xb1*v2)+b2*(v2-v1)-
     +               s1*(xb2*v3-xb1*v4)-b1*(v4-v3)       
      else if (k.eq.2) then
            cg=s2*(v2-v1)-s1*(v4-v3)
      else if (k.eq.3) then
            cg=-xb2*v1+xb1*v2+xb2*v3-xb1*v4
      else if (k.eq.4) then
            cg=-v2+v1+v4-v3
      else if (k.eq.5) then
            cg=b2*(xb2*v1-xb1*v2)-b1*(xb2*v3-xb1*v4)
      else if (k.eq.6) then
            cg=(s2-s1)*(xb2-xb1)
      else if (k.eq.7) then
            cg=(b2-b1)*(xb2-xb1)
            ...
            c3=-xb2*v1+xb1*v2+xb2*v3-xb1*v4
            c4=-v2+v1+v4-v3
            c6=(s2-s1)*(xb2-xb1)
            c7=(b2-b1)*(xb2-xb1)
            cg=c4*c7-c3*c6
      else if (k.eq.11) then
            c1=s2*(xb2*v1-xb1*v2)+b2*(v2-v1)-
     +               s1*(xb2*v3-xb1*v4)-b1*(v4-v3)       
            c5=b2*(xb2*v1-xb1*v2)-b1*(xb2*v3-xb1*v4)
            c6=(s2-s1)*(xb2-xb1)
            c7=(b2-b1)*(xb2-xb1)
            cg=c1*c7-c5*c6
      endif
           ...
           do 1720 j=1,nz
              ...
              if(iabs(igrid).eq.1) then
                ...
                if(iabs(igrid).eq.2) then
                  t1=(cg(layer,iblk,8)*xp+cg(layer,iblk,9)*xp**2
     +               +cg(layer,iblk,10)*zp+cg(layer,iblk,11))/
     +               (cg(layer,iblk,6)*xp+cg(layer,iblk,7))**2
                  t2=(cg(layer,iblk,3)+cg(layer,iblk,4)*xp)/
     +               (cg(layer,iblk,6)*xp+cg(layer,iblk,7)) 
