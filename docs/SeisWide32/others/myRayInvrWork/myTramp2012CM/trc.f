...
c zr(npt) is the depth of ray before entering the current block. z is ???.
c
...
c                 
      ...
      ar(npt,1)=y(2)
      ar(npt,2)=y(2)
      vp(npt,1)=vel(xr(npt),zr(npt), abs(ar(npt,1)) )
...
c      
      ...
      include 'tramp.com' 
c   
      vx=(cg(layer,iblk,8)*x+cg(layer,iblk,9)*x**2+cg(layer,iblk,10)*z+
     +    cg(layer,iblk,11))/(cg(layer,iblk,6)*x+cg(layer,iblk,7))**2
      vz=(cg(layer,iblk,3)+cg(layer,iblk,4)*x)/
     +   (cg(layer,iblk,6)*x+cg(layer,iblk,7))
      dstep=step*vel(x,z, ang_this)/(abs(vx)+abs(vz))
...
c            
      ...
      vel = vel2(x,z,ang, layer, iblk)
      return      
      end         
c                 


      function vel2(x,z, ang, i, j)
c                 
c     calculate p-wave velocity at point (x,z) in model
c      
      include 'tramp.par'
...
c      when the angle is 90 degrees, it means the ray is traveling horizontally.
     ...
     +       .and. ang_this .ge. 0.) then
      ...
      vel2 = velOld*bratio
      if (is_anisotropy.gt.0 .and. anisotropy(i) .gt. 0. 
     +       .and. j .eq. 1. .and. ang_this.gt.0.) then
          if ( abs(angInDegree-ang_print) .gt. 50.) then
      	    print*,'Layer,ang,v,v_ani:',i,angInDegree,velOld,vel2
      	    ang_print = angInDegree
      	  endif
      endif
...
c 
      ...
      term1=cg(layer,iblk,3)+cg(layer,iblk,4)*x
      term2=cg(layer,iblk,1)*x+cg(layer,iblk,2)*x*x+
     +       term1*y(1)+cg(layer,iblk,5)
      vxv=(cg(layer,iblk,8)*x+cg(layer,iblk,9)*x*x+
     +    cg(layer,iblk,10)*y(1)+cg(layer,iblk,11))/
     +    (term2*(cg(layer,iblk,6)*x+cg(layer,iblk,7)))
      vzv=term1/term2
      f(2)=vzv-vxv*f(1)
c
      if(.not.ok) then
        if(isrkc.eq.1.and.idump.eq.1) then
          write(12,5) vel(x,y(1), -1.)*(vxv**2+vzv**2)**.5
...
c 
      f(1)=tan(y(2))
      ang_this = abs(y(2))
      term1=cg(layer,iblk,3)+cg(layer,iblk,4)*y(1)
      term2=cg(layer,iblk,1)*y(1)+cg(layer,iblk,2)*y(1)*y(1)+
     +       term1*x+cg(layer,iblk,5)
      vxv=(cg(layer,iblk,8)*y(1)+cg(layer,iblk,9)*y(1)*y(1)+
     +    cg(layer,iblk,10)*x+cg(layer,iblk,11))/
     +    (term2*(cg(layer,iblk,6)*y(1)+cg(layer,iblk,7)))
      vzv=term1/term2
      f(2)=vzv*f(1)-vxv
c
      if(.not.ok) then
        if(isrkc.eq.1.and.idump.eq.1) then
      ang_this = -1
          write(12,5) vel(y(1),x, -1.)*(vxv**2+vzv**2)**.5
