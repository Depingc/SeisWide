     ...
     +5x,'phase') 
      if(nbnd.gt.1) then
        ...
        do 10 i=1,nbnd-1
           ...
           vp(np,1)=vel(xr(np),zr(np), abs(ar(np,1)) )
           vp(np,2)=vel(xr(np),zr(np), abs(ar(np,2)) )
	     vs(np,1)=vp(np,1)*vsvp(layer,iblk)
	     vs(np,2)=vp(np,2)*vsvp(layer,iblk)
...
c                 
      if(amprev.eq.0) then 
        ...
        xend=xr(npt)
           ...
           vp(npt,2)=vel(xr(npt),zr(npt), abs(ar(npt,2)) )
      	   vs(npt,1)=vp(npt,1)*vsvp(layer,iblk)
      	   vs(npt,2)=vp(npt,2)*vsvp(layer,iblk)
        vpe=vp(npt,1)
        vse=vs(npt,1) 
        lend=layer
        aend=aamp(nbnd,1)
