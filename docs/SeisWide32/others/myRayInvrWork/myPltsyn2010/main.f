...
c
     ...
     +   inmo,depthOBS,depthWater,depthGUN,idump,vred,sps,nsmth,scalef,
     +   scaleXmin,rcor,xnorm,amp,
...
c
      read(10,pltpar)
      read(10,axepar)
c......................better to set tmin = 0.0 otherwise it can have problem                 
      tmin = 0.0
      open(unit=11, file='sect.out', status='old')
      if(idump.eq.1) open(unit=12, file='syn.out')
      
c......for outputting to segy ... by Deping      
      open(unit=13, file='syn.sgy.data',status='unknown',form='binary')
c.....
      
      if(iwavlt.eq.2) open(unit=20, file='w.in', status='old')
      open(unit=19, file='p.ps')
c
