...
c
     ...
     +     hil(121),xomit(1),sp(ppseis),tp(ppseis),maxamp,minamp
      ...
      real xfill(ppseis),yfill(ppseis)
c... xfill and yfill arrays are introduced for plotting fill area by deping c   Mar 2008
...
1000  continue
c
c... namp needs to initialize to 0  otherwise VS2005 makes it a wrong value... by deping C, Mar 2008
      namp=0
c.... end of mod
     ...
     +  ' km exceeded ***'/)
      ...
      if(na(nseis).gt.0) then
        ...
        if(iscale.gt.0) then 
          ...
          if (scaleXmin .gt. 0. .and. range .ge. scaleXmin) then
            rr=(range/scaleXmin)**rcor
          end if
        end if    
        if (rr .lt. 1.) rr = 1.
        rr = rr * scalef
...
15         format(3e12.5)
           if(iscale.gt.0) then
             sect(nseis,j,2) = sect(nseis,j,2) * rr
             samp=abs(sect(nseis,j,2))
             if(samp.gt.maxamp) maxamp=samp
           end if 
           if(vred.gt.0.) then
                dxDeping = dist(nseis)-xshot

                if (inmo .eq. 2) then
                  tmpGUNDeping = depthWater - depthGUN
                  tmpOBSDeping = depthWater - depthOBS
                  tmpOffset2 = dxDeping*dxDeping*0.25
                  dtDeping = sqrt(tmpOffset2+tmpGUNDeping*tmpGUNDeping)+
     +               sqrt(tmpOffset2+tmpOBSDeping*tmpOBSDeping)-
     +               tmpGUNDeping
c	// note: the above is the John Shilmald display for NMO


                    dzDeping = depthOBS - depthGUN
                    dtDeping = sqrt(dxDeping*dxDeping+dzDeping*dzDeping)
     +                     -dzDeping
c	// note: the above is the gas hydrate display for NMO

                else
                    dtDeping = abs(dxDeping)
c                 sect(nseis,j,1)=
c     +            sect(nseis,j,1)-abs(dist(nseis)-xshot)/vred
                endif
                sect(nseis,j,1)= sect(nseis,j,1)-dtDeping*rvred
           end if 
190     continue  
      end if      
      go to 180   
...
c.......Deping ########################################################
         tminUnreduced = tmin+rvred*abs(dist(i)-xshot)
         write(13) dist(i),xshot,npts,tminUnreduced,sps,(s(j),j=1,npts)
c.......###############################################################
         if(namp.gt.0) then
           do 109 j=1,namp
...
490          continue
         ...
         if(iscale.gt.0) then
            sr = scalef
            if (range .gt. scaleXmin) then
                sr = sr * (range / scaleXmin)**rcor
            end if
            do 610 j=1,npts
               s(j)=sr*s(j)
610         continue
         end if   
         if(clip.gt.0..and.iscale.gt.0) then
           do 590 j=1,npts
...
c.... This code is for fill area, added by Deping C.  Mar 2008###################
         if(ishade.eq.1) then
            ...
            call airvar(sp, tp, jstart, jend, tmean)
         end if
