...
c                 
c        31 -- output: velocity model sampled on a unifrom grid
c                 
...
c 
	program   rayinvr
c      subroutine rayinvr
c....Added by deping, vname to be passed into c++.
c   !MS$ATTRIBUTES DLLEXPORT :: RAYINVR

c...These two lines are used by Quick Win appl of Digital Fortran only.
	USE DFLIB
	USE DFNLS
...
c                 
     ...
     +        ifo2pt(pnobsf),ipos(pr2pt),modi(player)
      character*1 flag,ifmt0*1,ifmt1*4,form*20,cn*10
c
      include 'rayinvr.com'                 
c                 
...
c                 
     ...
     +                  crit,hws,npbnd,nbsmth,nhray,frbnd,
...
c
	INTEGER(4) response
      ...
      i33=0
c
c	OPEN (*, FILE= 'USER', TITLE= 'Screen display')
...
c                 
      read(10,pltpar)
      write(11,pltpar)
	write(*,*) "Finished reading pltpar group"
      read(10,axepar)
      write(11,axepar)
	write(*,*) "Finished reading axepar group"
      read(10,trapar)
      write(11,trapar)
	write(*,*) "Finished reading trapar group"
      read(10,invpar)
      write(11,invpar)
	write(*,*) "Finished reading invpar group"
c
c.....deping's modification:
c....this will always plot ray diagrams.
	iplot = 0
	itxout=1
ccc      open(unit=88, file='rPath.out', status='unknown')
	open(88,file="rPath.out",status='unknown',form='binary')
c this new file is to store raypath info.
c.....end of deping's mod.
...
c        open(unit=20, file='v.bin', status='old')
	open(20,file="v.bin",status='old',form='binary')
        iunit=20
      end if
c
...
c
	read(iunit)ncont,(nzed(i),i=1,ncont)
	read(iunit)((xm(icont,j),j=1,nzed(icont)),icont=1,ncont)
	read(iunit)((zm(icont,j),j=1,nzed(icont)),icont=1,ncont)
	do icont=1,ncont
		do j=1,nzed(icont)
		ivarz(icont,j) = 0
		end do
		do j=nzed(icont)+1,ppcntr
			xm(icont,j)=0.
			zm(icont,j)=0.
			ivarz(icont,j)=0
		enddo
	end do
	read(iunit)nlayer,(nvel(i,1),i=1,nlayer),(nvel(i,2),i=1,nlayer)
	read(iunit)((xvel(icont,j,1),j=1,nvel(icont,1)),icont=1,nlayer)
	read(iunit)((vf(icont,j,1),j=1,nvel(icont,1)),icont=1,nlayer)
	do icont=1,nlayer
		do j=1,nvel(icont,1)
		ivarv(icont,j,1) = 0
		end do
		do j=nvel(icont,1)+1,ppvel
			xvel(icont,j,1) = 0. 
			vf(icont,j,1) = 0. 
			ivarv(icont,j,1) = 0
		end do
	end do
	read(iunit)((xvel(icont,j,2),j=1,nvel(icont,2)),icont=1,nlayer)
	read(iunit)((vf(icont,j,2),j=1,nvel(icont,2)),icont=1,nlayer)
	do icont=1,nlayer
		do j=1,nvel(icont,2)
		ivarv(icont,j,2) = 0
		end do
		do j=nvel(icont,2)+1,ppvel
			xvel(icont,j,2) = 0. 
			vf(icont,j,2) = 0. 
			ivarv(icont,j,2) = 0
		end do
	...
	print *, 'ncont,nlayer: ',ncont,nlayer
	print *, '1st layer:',xm(1,1),zm(1,1)
	print *, '2nd layer:',xm(2,1),zm(2,1),xm(2,nzed(2)),zm(2,nzed(2))
	print *, 'last layer:',xm(ncont,1),zm(ncont,1),xm(ncont,
     &	nzed(ncont)),zm(ncont,nzed(ncont))
	print*,(nzed(j),j=1,ncont)
      nrzmax=ppcntr/10
      nrvmax=ppvel/10
c     
c     open I/O units
c
      ...
      if(modout.gt.0) open(unit=31, file='v.out')
...
c
c  Deping's modification: used to open with status='Old'.
c  this causes the program to stop when idata=1 and 
c  yet tx.in doesn't exists.
        open(unit=26, file='tx.in')
c
        isf=1
        nsf=0
        xshotc=0.
910     read(26,930,end=916) xpf(isf),tpf(isf),upf(isf),ipf(isf)
930     format(3f10.3,i10)
	...
	goto 917
916	print *,' Problem: idata=1 but cannot load tx.in file; Resetting!'
	idata=0
	invr=0

      response = MESSAGEBOXQQ('Cannot load tx.in file as you asked by 
     +using idata=1 or invr=1!'C, 'FYI?'C, 
     + MB$RETRYCANCELQWIN .OR. MB$DEFBUTTON2)

917	continue
c  end of Deping's modification.
c
...
c       Dal modification
        iord=int(alog10(real(npfref(nfrefl))))+1
        write(ifmt0,'(i1)') iord
        ifmt1='(i'//ifmt0//')'
        write(cn,ifmt1) npfref(nfrefl)

        form='(3x,'//cn(1:iord)//'f7.2)'
        read(30,form) (xfrefl(nfrefl,i),i=1,npfref(nfrefl))
        read(30,form) (zfrefl(nfrefl,i),i=1,npfref(nfrefl))

        form='(3x,'//cn(1:iord)//'i7)'
        read(30,form) (ivarf(nfrefl,i),i=1,npfref(nfrefl))
c       end Dal mod

c        read(30,555) (xfrefl(nfrefl,i),i=1,npfref(nfrefl))
c        read(30,555) (zfrefl(nfrefl,i),i=1,npfref(nfrefl))
c        read(30,575) (ivarf(nfrefl,i),i=1,npfref(nfrefl))
c555     format(3x,<npfref(nfrefl)>f7.2)
c575     format(3x,<npfref(nfrefl)>i7)
...
c                 
c     assign switch iconv and check array ncbnd for array
c     values greater than pconv
c                 
      do 470 i=1,prayf 
...
c
               if(vr(npt,2).gt.0.and.irs.gt.0.and.modout.eq.2) 
...
c
      if(modout.gt.0) call modwr(modout,dxmod,dzmod,modi,ifrbnd,frz,
     +                           xmmin,xmmax)
c                 
900   ntblk=0     
      do 920 i=1,nlayer
...
925   format('|           model consists of ',i2,' layers and ',i4,
     +       ' blocks           |'/,'|',64x,'|'/
     +'|----------------------------------------------------------',
     +'------|'/) 
c
...
c
            write(11,868)
868         format('     shot  dir   npts   Trms   chi-squared'/
     +             '------------------------------------------')
...
c
	close(88) 
c...this unit is added by deping for raypath info.
      response = MESSAGEBOXQQ('Ray tracing completed successfully!'C,
     + 'Congratulations !'C, 
     +  MB$RETRYCANCELQWIN .OR. MB$DEFBUTTON2)
      stop        
c                 
999   write(6,105) 
105   format(/'***  error in velocity model from main ***'/)
c                 
      go to 9999  
c
      end         
