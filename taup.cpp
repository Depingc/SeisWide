	...
	for (ip=0, p=pmin; ip<np; p=pmin+(++ip)*dp) {
		...
		for (ix=0, x=xmin; ix<nx; x=xmin+(++ix)*dx) {
			...
			for (it=fit; it<lit; it++) {
				out_traces[ip][it-id]+= (float)fabs(dx)*
				    (traces[ix][it]+
...
// *headOut will be reset based on input np
void fwd_tx_sstack(HEAD_READ *headRead, HEAD_READ *headOut, int np, float pmin, float dp, 
	bool isDist)
{
	int ip;		/* loop counters */
	float p;		/* auxilairy variables */

	int nt = headRead->getns();
	float dt = (float)headRead->getdt();
	int nx = headRead->getnx();

	SegyWrapper::headReadCopyReset(headRead, headOut, np);

	for (ip = 0, p = pmin; ip < np; p = pmin + (++ip)*dp) {
		if (isDist)
			headOut->trArr[ip].setDistance((float)p);
		else
			headOut->trArr[ip].setOffset((float)p);
	}

	return fwd_tx_sstack(headRead, headOut, isDist);
}

// this function will need *headIn and *headOut already have arrays properly assigned. 
// This is required by inverse Taup transform
void fwd_tx_sstack(HEAD_READ *headIn, HEAD_READ *headOut, bool isDist)
{
	int ip, ix, it;		/* loop counters */
	float x, p;		/* auxilairy variables */
	int fit, lit;		/* first and last time samples */
	...
	int nt = headIn->getns();
	float dt = (float)headIn->getdt();
	// int nx = headIn->nxmax;
	// int np = headOut->nxmax;
	float xmin = isDist ? headIn->trArr[0].getDistance() : headIn->trArr[0].getOffset();
	float tmin0 = headIn->trArr[0].getTraceDelay();

	/* loop over slopes */
	// first reset data
	for (ip = 0; ip < headOut->getnx(); ip++) {
		memset(headOut->trArr[ip].data, (int) '\0', nt*sizeof(float));
	}

	for (ip = 0; ip<headOut->getnx(); ip++) {
		p = isDist ? headOut->trArr[ip].getDistance() : headOut->trArr[ip].getOffset();
		for (ix = 0; ix<headIn->getnx(); ix++) {
			if (isDist) {
				x = headIn->trArr[ix].getDistance();
				x -= xmin;
			}
			else {
				x = headIn->trArr[ix].getOffset();
			}
			// ;

			/* compute two point interpolator */
			delay = (p*x - headIn->trArr[ix].getTraceDelay() + tmin0) / dt;
			// delay = p*x / dt;  // original value from CWP
			if (delay >= 0) {
				id = (int)delay;
				fit = id + 1;
				lit = nt - 1;
			...
			/* compute the actual slant stack */
			for (it = fit; it<lit; it++) {
				headOut->trArr[ip].data[it - id] +=
					(headIn->trArr[ix].data[it] +
					dfrac*(headIn->trArr[ix].data[it + 1] - headIn->trArr[ix].data[it]));
			}
		}
	}
}
...

{
	...
	fftscl = 1.0f / (float)ntaufft;
	nkfft = npfa(4*np);

	/* determine frequency and wavenumber sampling */
	nw = ntaufft/2+1;
	dw = 2.0f*PI / (float)(ntaufft*dtau);
	fw = fk = 0.0f;
	dk = 2.0f*PI / (float)(nkfft*dx);
	nkmax = (int)(PI*pmax/(dt*dk) + 1);
...
void inv_tx_sstack(HEAD_READ *headTaup, HEAD_READ *headTx, int npoints, bool isDist)
{
	int ip;			/* loop counter */
	int np2 = npoints / 2;	/* half number of points in rho filter */
	float p;
	...
	/* allocate space */
	int nt = headTaup->getns();
	float dt = (float)headTaup->getdt();
	int np = headTaup->getnx();
	...
	/* convolve input traces with rho filter */
	for (ip = 0; ip<np; ip++)
		convolve_cwp(nt, -np2, headTaup->trArr[ip].data, npoints, 0, rho, nt, 0, rho_traces[ip]);

	// now memcpy rho_traces[p][t]  to  headTaup->trArr[x][t] with p reversed in sign
	// Note that all traces need to be reversed: the first trace to be come the last,
	segydynamic *trArrBac = (segydynamic *)xmalloc(sizeof(segydynamic) * np);
	for (ip = 0; ip<np; ip++) {
		memcpy((void *)&trArrBac[ip].tracl, (void *)&headTaup->trArr[ip].tracl, 240L);
	}
	for (ip = 0; ip<np; ip++) {
		memcpy((void *)&headTaup->trArr[ip].tracl, (void *)&trArrBac[np - 1 - ip].tracl, 240L);
		headTaup->trArr[ip].tracl = trArrBac[ip].tracl;
		memcpy((void *)&headTaup->trArr[ip].data[0], rho_traces[np - 1 - ip], nt*sizeof(float));

		/* inverse transform == forward transform with negative slopes */
		if (isDist) {
			p = headTaup->trArr[ip].getDistance();
			headTaup->trArr[ip].setDistance(-p);
		}
		else {
			p = headTaup->trArr[ip].getOffset();
			headTaup->trArr[ip].setOffset(-p);
		}
	}
	...
	// and we will output headTx[x][t]
	//fwd_tx_sstack(dt, nt, np, -pmin, -dp, nx, xmin, dx, rho_traces,
	//	out_traces);
	fwd_tx_sstack(headTaup, headTx, isDist);
...
******************************************************************************/
{
	...
	/* oddness of npoints */
	if (2 * nph == npoints)   AfxMessageBox (_T("npoints must be odd!\n"));

	/* compare filter length with number of time samples */
	if (nph > nt)   
		AfxMessageBox(_T("filter length larger than number of time samples!"));

	/* compute padding factor */
	if (nt > maxnpfa)   
		AfxMessageBox(_T("number of time samples too large for npfa!"));
	ntfft = npfa(nt);
...
******************************************************************************/
{
	...
	int ltaper;
        ...
        /* define variables */
        ntfft=npfar(nt);
        fac = 1000.f*gofx(igopt,offref,interoff,depthref);
        pmin /=fac;
        pmax /=fac;
        dp /=fac;
	pmula /=fac;
	pmulb /=fac;
	ipa = (int)((pmula - pmin) / dp);
	ipb = (int)((pmulb - pmin) / dp);
	if (ipa<0) ipa=0;
	ltaper=7;
	np = 1 + (int)((pmax - pmin) / dp);
	...
	for(iw=0;iw<nw;iw++) {
		...
		if(wa>0.) {
			...
			r[0].r *= (1.f+prewhite);	/* apply prewhitening */
			for(rsum=0.,ip=1;ip<np;ip++) 
				rsum += (float)sqrt(r[ip].r*r[ip].r + r[ip].i*r[ip].i);
			rsum=rsum/r[0].r;  
...
******************************************************************************/
{
	...
	nmax=max(nmax,(int)(nx*sizeof(complex)));
	...
	for(iw=0;iw<nw;iw++) {
		...
		if(wa>0.) {
			...
			for(ix=0;ix<nx;ix++) {
			    ...
			    for(ip=0;ip<np;ip++) {
				p = pmin + ip*dp;
				tr = (float)cos(w*p*g[ix]);
				ti = (float)sin(w*p*g[ix]);
				dr = ctemp[ip].r;
				di = ctemp[ip].i;
				rsum += tr*dr - ti*di;
				isum += tr*di + ti*dr;
...
******************************************************************************/
{
	...
	if(igopt==2) {
		return (float)(sqrt(refdepth*refdepth+offset*offset));
	}
	if(igopt==3) {
		return(offset);
	}
	if(igopt==4) {
		return (float)(fabs(offset));
	}
	if(igopt==5) {
		return (float)(sqrt(refdepth*refdepth+offset*offset)-refdepth);
	} else {
		return 0.0f;
	}
}
...
******************************************************************************/
{
	...
	for(j=0;j<np;j++) {
		...
		r[j].r    = (float)(fac*rsum);
		r[j].i    = (float)(fac*isum);
	}
}
...
{
	int ip, ix;
	double p, rsum, isum, dr, di, tr, ti, fac;
	fac = (1. / nx);
	for(ip=0;ip<np;ip++) {
		p = pmin + ip*dp;
		rsum = isum = 0.;
		...
		rhs[ip].r   = (float)(fac*rsum);
		rhs[ip].i   = (float)(fac*isum);
	}
}
...
******************************************************************************/
{
	...
	float	df, dff, wa, wb, dxx, eps = (float)1.0e-30, f, fcl, fch;
	...
	dff = (float)(2.*PI / ntfft);
	for(ix=0;ix<nx;ix++) {
		for(it=0;it<nt;it++) {
			rt[it]=in_traces[ix][it];
			a[it]= (float)fabs(rt[it]);
		}
		runav(nt,lagc,a,b);
		runav(nt,lagc,b,a);
