...
***************************************************************************/
{
	...
	vnd = (VND *)VNDemalloc(sizeof(VND),"VNDop: allocating VND");
...
*****************************************************************/
{
	...
	ib = fseek(vnd->FileDescriptor,(off_t) ib,SEEK_SET);
	if(ib!=0){
...
void *VNDemalloc(size_t n, char *s){
	...
	long *j;
	n=1+(n-1)/sizeof(long);
	if((p = xmalloc((n+3)*sizeof(long)))==NULL) {
	...
	pp = &j[2];
	j[n+2]=1234;
	total_VND_mem += (long)( (n + 3)*sizeof(long) );
	return (pp);
}
void VNDmemchk( void *p , char * msg) 
{
...
#endif
{
	...
	VNDrw(iop,iopanel,vnd[0],idim,key,ipanel, (char *)values,
