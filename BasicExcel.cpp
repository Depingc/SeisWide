...
// PROMISE: Changes will not be written to compound file.
{
	...
	{for (size_t i=0; i<109 && header_.BATArray_[i]!=-1; ++i)
	{
		...
		for (size_t j=0; j<maxIndices; ++j)
		{
			if ((size_t)header_.BATArray_[i] >= indices[j] &&
...
// EXPLAIN: property contains information on the type of property to be created.
{
	...
	if (propertyname != 0)
	{
		...
		delete[] propertyname;
		property->nameSize_ = (WORD)(propertynameLength*2+2);
...
WORD Worksheet::CellTable::RowBlock::CellBlock::RowIndex()
{
	switch (type_)
	{
		case CODE::BLANK:
...
WORD Worksheet::CellTable::RowBlock::CellBlock::ColIndex()
{
	switch (type_)
	{
		case CODE::BLANK:
...
bool BasicExcel::SaveAs(CString filename)
{
	if (file_.IsOpen()) file_.Close();
...
BasicExcelWorksheet* BasicExcel::AddWorksheet(int sheetIndex)
{
	...
	{
		char sname[50];
		sprintf(sname, "Sheet%zd", sheetNo++);
		...
		for (size_t j=0; j<maxRowBlocks_; ++j) 
		{
			...
			{for (size_t k=0; k<maxRows; ++k)
			{
				for (; l<maxCellBlocks; ++l)
				{
					...
					{
						worksheets_[i].cellTable_.rowBlocks_[j].dbcell_.offsets_[k] = (WORD)cellOffset;
						break;
					}
...
void BasicExcel::UpdateWorksheets()
{
	...
	for (size_t s=0; s<maxWorksheets; ++s)
	{
		...
		worksheets_[s].dimensions_.lastUsedRowIndexPlusOne_ = (WORD)maxRows;
		worksheets_[s].dimensions_.lastUsedColIndexPlusOne_ = (WORD)maxCols;
		...
		for (size_t c=0; c<maxCols_; ++c)
		{
			...
			switch (cell->Type())
			{
				...
				case BasicExcelCell::WSTRING:
				{
					...
					os << textQualifier << cell->GetCString() << textQualifier;
					break;
				}
			}
...
// Before Excel 2007, size must be less than 65535 or col exceeds 255.
// From Excel 2007 and later, Total number of rows and columns on a worksheet 1,048,576 rows by 16,384 columns
...
// this is not working.... Deping
CString BasicExcelCell::GetCString() const
{
	CString t = CString(&*(wstr_.begin()));
	vector<wchar_t> wstr(wstr_.size());
	if (!wstr.empty() && Get(&*(wstr.begin()))) 
		return CString(&*(wstr_.begin()));
	else return _T("");
}
